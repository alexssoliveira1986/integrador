...
<html lang="pt-br">
	<head>
		...
		<hr>
		<strong>Consulta NF</strong><hr></br>
		
	<?php
	require ("select-nf-1.php");
	$unidade = $ncontrolenfh3;
	$nota = $ncontrolenfh2;
	...
	if ($row['NFHSTS'] == 'G'){
	echo '<hr>' . '<h3 style="background-color:DeepSkyBlue;">' . 'Nota Fiscal Validada na Receita ' . '</h3>';
	}else echo '<hr>' . '<h3 style="background-color:Coral;">' . 'Nota Fiscal Não Validada na Receita ' . '</h3>';
	...
	/*$sql5 = ("select * from tb_pcadoi where doinum='$nota' and doiemi='$unidade' ");
	foreach ($conn->query($sql5) as $row5 ) {
	if ($row5['DOINUM']=($row['NFHNNF'])) {*/
	...
	else echo '<strong>' . 'Nota Fiscal: ' . '</strong>' . $nota . '<strong>' . ' Serie: ' . '</strong>' . ' 55 ' . '<strong>' . 'Unidade: ' . '</strong>' . $unidade . '<strong>'. '<p style="color:red;"> Não Localizada '.'</strong>' . '<hr>';
		
	}else echo '<strong>' . 'Nota Fiscal: ' . '</strong>' . $nota . '<strong>' . ' Serie: ' . '</strong>' . ' 55 ' . '<strong>' . 'Unidade: ' . '</strong>' . $unidade . '<strong>' . '<p style="color:red;"> Não Localizada '.'</strong>'.'<br>'  ; 
	...
	if ($row['NFHSTS'] == 'G'){		
	//echo '<td><a href="atual.php" target="_self"> <button type="button" onclick="submit()">Atualizar</button></a></td>';
	echo '<td><a href="num.php" target="_self"><button type="button" onclick="submit()">Baixar XML</button></a></td>';	
	
	
			$campo1 = '-nfe.xml';
			$chavedeacesso = $nchave.$campo1;
			$pasta = 'U:\/';
